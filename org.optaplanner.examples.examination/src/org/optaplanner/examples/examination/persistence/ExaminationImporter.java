...
public class ExaminationImporter extends AbstractTxtSolutionImporter {
	...
	public static class ExaminationInputBuilder extends TxtInputBuilder {
		...
		public Solution readSolution() throws IOException {
			...
			int possibleForOneExamSize = examination.getPeriodList().size()
					* examination.getRoomList().size();
			BigInteger possibleSolutionSize = BigInteger.valueOf(
					possibleForOneExamSize).pow(
					examination.getExamList().size());
			logger.info(
					"Examination {} has {} students, {} exams, {} periods, {} rooms, {} period constraints"
							+ " and {} room constraints with a search space of {}.",
					getInputId(), examination.getStudentList().size(),
					examination.getExamList().size(), examination
							.getPeriodList().size(), examination.getRoomList()
							.size(), examination.getPeriodPenaltyList().size(),
		...
		private void readTopicListAndStudentList() throws IOException {
			...
			for (int i = 0; i < examSize; i++) {
				...
				for (int j = 1; j < lineTokens.length; j++) {
					topicStudentList.add(findOrCreateStudent(studentMap,
							Integer.parseInt(lineTokens[j])));
				}
		...
		private Student findOrCreateStudent(Map<Integer, Student> studentMap,
				int id) {
			...
			for (int i = 0; i < periodSize; i++) {
				...
				if (lineTokens.length != 4) {
					throw new IllegalArgumentException("Read line (" + line
							+ ") is expected to contain 4 tokens.");
				}
				String startDateTimeString = lineTokens[0] + " "
						+ lineTokens[1];
		...
		private void readPeriodPenaltyList() throws IOException {
			...
			while (!line.equals("[RoomHardConstraints]")) {
				...
				if (lineTokens.length != 3) {
					throw new IllegalArgumentException("Read line (" + line
							+ ") is expected to contain 3 tokens.");
				}
				PeriodPenalty periodPenalty = new PeriodPenalty();
				periodPenalty.setId((long) id);
				id++;
				Topic leftTopic = topicList
						.get(Integer.parseInt(lineTokens[0]));
				periodPenalty.setLeftTopic(leftTopic);
				PeriodPenaltyType periodPenaltyType = PeriodPenaltyType
						.valueOf(lineTokens[1]);
				periodPenalty.setPeriodPenaltyType(periodPenaltyType);
				Topic rightTopic = topicList.get(Integer
						.parseInt(lineTokens[2]));
				...
				case EXAM_COINCIDENCE:
					if (leftTopic.getId().equals(rightTopic.getId())) {
						logger.warn("  Filtering out periodPenalty ("
								+ periodPenalty
								+ ") because the left and right topic are the same.");
						ignorePenalty = true;
					} else if (!Collections.disjoint(
							leftTopic.getStudentList(),
							rightTopic.getStudentList())) {
						throw new IllegalStateException("PeriodPenalty ("
								+ periodPenalty + ") for leftTopic ("
								+ leftTopic + ") and rightTopic (" + rightTopic
								+ ")'s left and right topic share students.");
					} else if (coincidenceMap.get(leftTopic).contains(
							rightTopic)) {
						logger.trace("  Filtering out periodPenalty ("
								+ periodPenalty + ") for leftTopic ("
								+ leftTopic + ") and rightTopic (" + rightTopic
								+ ") because it is mentioned twice.");
						ignorePenalty = true;
					} else {
						boolean added = coincidenceMap.get(leftTopic).add(
								rightTopic)
								&& coincidenceMap.get(rightTopic)
										.add(leftTopic);
						if (!added) {
							throw new IllegalStateException(
				...
				case EXCLUSION:
					if (leftTopic.getId().equals(rightTopic.getId())) {
						...
						boolean added = exclusionMap.get(leftTopic).add(
								rightTopic)
					...
					if (afterMap.get(leftTopic).contains(rightTopic)) {
						...
						if (!added) {
							throw new IllegalStateException(
									"The periodPenaltyType ("
											+ periodPenaltyType
											+ ") for leftTopic (" + leftTopic
											+ ") and rightTopic (" + rightTopic
			...
			for (Map.Entry<Topic, Set<Topic>> entry : coincidenceMap.entrySet()) {
				...
				for (Topic middleTopic : new ArrayList<Topic>(middleTopicSet)) {
					for (Topic rightTopic : new ArrayList<Topic>(
							coincidenceMap.get(middleTopic))) {
						if (rightTopic != leftTopic
							...
							indirectPeriodPenalty
									.setPeriodPenaltyType(PeriodPenaltyType.EXAM_COINCIDENCE);
							...
							boolean added = coincidenceMap.get(leftTopic).add(
									rightTopic)
									&& coincidenceMap.get(rightTopic).add(
											leftTopic);
							if (!added) {
								throw new IllegalStateException(
										"The periodPenalty ("
			...
			for (Map.Entry<Topic, Set<Topic>> entry : afterMap.entrySet()) {
				...
				while (!queue.isEmpty()) {
					...
					if (!afterLeftSet.contains(rightTopic)) {
						...
						id++;
						indirectPeriodPenalty
								.setPeriodPenaltyType(PeriodPenaltyType.AFTER);
						indirectPeriodPenalty.setLeftTopic(leftTopic);
						indirectPeriodPenalty.setRightTopic(rightTopic);
						periodPenaltyList.add(indirectPeriodPenalty);
						boolean added = afterMap.get(leftTopic).add(rightTopic);
		...
		private void readRoomPenaltyList() throws IOException {
			...
			while (!line.equals("[InstitutionalWeightings]")) {
				...
				roomPenalty.setTopic(topicList.get(Integer
						.parseInt(lineTokens[0])));
				roomPenalty.setRoomPenaltyType(RoomPenaltyType
						.valueOf(lineTokens[1]));
		...
		private int readHeaderWithNumber(String header) throws IOException {
			...
			if (!line.startsWith("[" + header + ":") || !line.endsWith("]")) {
				throw new IllegalStateException("Read line (" + line
						+ " is not the expected header ([" + header
						+ ":number])");
			}
			return Integer.parseInt(line.substring(header.length() + 2,
					line.length() - 1));
		}
		...
		private void readInstitutionalWeighting() throws IOException {
			...
			institutionParametrization.setTwoInARowPenalty(Integer
					.parseInt(lineTokens[1]));
			lineTokens = readInstitutionalWeightingProperty("TWOINADAY", 2);
			institutionParametrization.setTwoInADayPenalty(Integer
					.parseInt(lineTokens[1]));
			lineTokens = readInstitutionalWeightingProperty("PERIODSPREAD", 2);
			institutionParametrization.setPeriodSpreadLength(Integer
					.parseInt(lineTokens[1]));
			institutionParametrization.setPeriodSpreadPenalty(1); // constant
			lineTokens = readInstitutionalWeightingProperty(
					"NONMIXEDDURATIONS", 2);
			institutionParametrization.setMixedDurationPenalty(Integer
					.parseInt(lineTokens[1]));
			lineTokens = readInstitutionalWeightingProperty("FRONTLOAD", 4);
			institutionParametrization.setFrontLoadLargeTopicSize(Integer
					.parseInt(lineTokens[1]));
			institutionParametrization.setFrontLoadLastPeriodSize(Integer
					.parseInt(lineTokens[2]));
			institutionParametrization.setFrontLoadPenalty(Integer
					.parseInt(lineTokens[3]));
			examination
					.setInstitutionParametrization(institutionParametrization);
		}
		...
		private String[] readInstitutionalWeightingProperty(String property,
			...
			if (!lineTokens[0].equals(property)
					|| lineTokens.length != propertySize) {
				throw new IllegalArgumentException("Read line ("
						+ Arrays.toString(lineTokens)
						+ ") is expected to contain " + propertySize
						+ " tokens and start with " + property + ".");
			}
			return lineTokens;
		}
		...
		private void tagFrontLoadLargeTopics() {
			...
			Collections.sort(sortedTopicList, new Comparator<Topic>() {
				public int compare(Topic a, Topic b) {
					return new CompareToBuilder()
							...
							.append(b.getId(), a.getId()) // Descending
															// (according to
															// spec)
							.toComparison();
				}
			});
			int frontLoadLargeTopicSize = examination
					.getInstitutionParametrization()
					.getFrontLoadLargeTopicSize();
			if (frontLoadLargeTopicSize == 0) {
				return;
			}
			int minimumTopicId = sortedTopicList.size()
					- frontLoadLargeTopicSize;
			if (minimumTopicId < 0) {
				logger.warn("The frontLoadLargeTopicSize ("
						+ frontLoadLargeTopicSize
			...
			for (Topic topic : sortedTopicList.subList(minimumTopicId,
					sortedTopicList.size())) {
		...
		private void tagFrontLoadLastPeriods() {
			...
			int frontLoadLastPeriodSize = examination
					.getInstitutionParametrization()
			...
			if (minimumPeriodId < 0) {
				logger.warn("The frontLoadLastPeriodSize ("
			...
			for (Period period : periodList.subList(minimumPeriodId,
					periodList.size())) {
		...
		private void createExamList() {
			...
			Map<Topic, LeadingExam> leadingTopicToExamMap = new HashMap<Topic, LeadingExam>(
					topicList.size());
			for (Topic topic : topicList) {
				...
				if (leadingTopic == topic) {
					...
					leadingExam
							.setFollowingExamList(new ArrayList<FollowingExam>(
									10));
					leadingTopicToExamMap.put(topic, leadingExam);
					exam = leadingExam;
				} else {
					FollowingExam followingExam = new FollowingExam();
					LeadingExam leadingExam = leadingTopicToExamMap
							.get(leadingTopic);
					if (leadingExam == null) {
						throw new IllegalStateException("The followingExam ("
								+ topic.getId() + ")'s leadingExam ("
								+ leadingExam + ") cannot be null.");
					}
