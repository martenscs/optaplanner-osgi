...
public class BedDesignationDifficultyWeightFactory implements
	...
	public Comparable createSorterWeight(PatientAdmissionSchedule schedule,
		...
		for (Room room : schedule.getRoomList()) {
			hardDisallowedCount += (room
					.countHardDisallowedAdmissionPart(bedDesignation
							.getAdmissionPart()) * room.getCapacity());
			softDisallowedCount += (room
					.countSoftDisallowedAdmissionPart(bedDesignation
							.getAdmissionPart()) * room.getCapacity());
		}
		return new BedDesignationDifficultyWeight(bedDesignation,
				hardDisallowedCount, softDisallowedCount);
	}

	public static class BedDesignationDifficultyWeight implements
			Comparable<BedDesignationDifficultyWeight> {
		...
		public BedDesignationDifficultyWeight(BedDesignation bedDesignation,
			...
			requiredEquipmentCount = bedDesignation.getPatient()
					.getRequiredPatientEquipmentList().size();
		...
		public int compareTo(BedDesignationDifficultyWeight other) {
			return new CompareToBuilder()
					...
					.append(hardDisallowedCount * nightCount,
							other.hardDisallowedCount * other.nightCount)
					.append(nightCount, other.nightCount)
					.append(softDisallowedCount * nightCount,
							other.softDisallowedCount * nightCount)
					// Descending (earlier nights are more difficult) // TODO
					// probably because less occupancy
					.append(other.bedDesignation.getAdmissionPart()
							.getFirstNight().getIndex(),
							bedDesignation.getAdmissionPart().getFirstNight()
									.getIndex())
					.append(bedDesignation.getId(),
							other.bedDesignation.getId()).toComparison();
		}
