...
import org.optaplanner.osgi.common.persistence.AbstractTxtSolutionExporter;

public class PatientAdmissionScheduleExporter extends
	...
	public static class PatientAdmissionScheduleOutputBuilder extends
			TxtOutputBuilder {
			...
			for (Patient patient : patientAdmissionSchedule.getPatientList()) {
				...
				for (BedDesignation bedDesignation : patientAdmissionSchedule
					...
					if (bedDesignation.getPatient().equals(patient)) {
						for (int i = 0; i < bedDesignation.getAdmissionPart()
								.getNightCount(); i++) {
							bufferedWriter.write(" "
									+ Long.toString(bedDesignation.getBed()
											.getId()));
						}
					}
				}
