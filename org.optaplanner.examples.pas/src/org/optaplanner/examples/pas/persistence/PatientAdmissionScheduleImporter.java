...
import org.optaplanner.osgi.common.domain.PersistableIdComparator;
import org.optaplanner.osgi.common.persistence.AbstractTxtSolutionImporter;

public class PatientAdmissionScheduleImporter extends
	...
	public static class PatientAdmissionScheduleInputBuilder extends
			TxtInputBuilder {
		...
		public Solution readSolution() throws IOException {
			...
			// TODO not all nights are planned, only the "planning horizon"
			// nights are planned
			BigInteger possibleSolutionSize = BigInteger.valueOf(
					patientAdmissionSchedule.getBedList().size()).pow(
			...
			logger.info(
					...
					getInputId(), patientAdmissionSchedule.getSpecialismList()
							.size(), patientAdmissionSchedule
							.getEquipmentList().size(),
		...
		private void readSpecialismList() throws IOException {
			...
			List<Specialism> specialismList = new ArrayList<Specialism>(
					specialismListSize);
			idToSpecialismMap = new HashMap<Long, Specialism>(
					specialismListSize);
		...
		private void readDepartmentListAndDepartmentSpecialismList()
				throws IOException {
			readConstantLine("DEPARTMENTS:");
			List<Department> departmentList = new ArrayList<Department>(
					departmentListSize);
			idToDepartmentMap = new HashMap<Long, Department>(
					departmentListSize);
			...
			for (int i = 0; i < departmentListSize; i++) {
				...
				if (departmentSpecialismTokens.length % 2 != 0) {
					throw new IllegalArgumentException(
							"Read line ("
									+ line
									+ ") is expected to contain even number of tokens ("
									+ departmentSpecialismTokens.length
									+ ") after 1st pipeline (|) separated by a space ( ).");
				}
				for (int j = 0; j < departmentSpecialismTokens.length; j += 2) {
					long specialismId = Long
							.parseLong(departmentSpecialismTokens[j + 1]);
					if (specialismId != 0) {
						...
						departmentSpecialism.setPriority(Integer
								.parseInt(departmentSpecialismTokens[j]));
						Specialism specialism = idToSpecialismMap
								.get(specialismId);
						if (specialism == null) {
							throw new IllegalArgumentException("Read line ("
									+ line
									+ ") has a non existing specialismId ("
									+ specialismId + ").");
						}
			...
			patientAdmissionSchedule
					.setDepartmentSpecialismList(departmentSpecialismList);
		}
		...
		private void readEquipmentList() throws IOException {
			...
			List<Equipment> equipmentList = new ArrayList<Equipment>(
					equipmentListSize);
			indexToEquipmentMap = new HashMap<Integer, Equipment>(
					equipmentListSize);
		...
		private void readRoomListAndRoomSpecialismListAndRoomEquipmentList()
				throws IOException {
			readConstantLine("ROOMS:");
			List<Room> roomList = new ArrayList<Room>(roomListSize);
			idToRoomMap = new HashMap<Long, Room>(roomListSize);
			List<RoomSpecialism> roomSpecialismList = new ArrayList<RoomSpecialism>(
					roomListSize * 5);
			...
			for (int i = 0; i < roomListSize; i++) {
				...
				Department department = idToDepartmentMap.get(Long
						.parseLong(lineTokens[2]));
				room.setDepartment(department);
				room.setGenderLimitation(GenderLimitation
						.valueOfCode(lineTokens[3]));
				...
				if (roomSpecialismTokens.length % 2 != 0) {
					throw new IllegalArgumentException(
							"Read line ("
									+ line
									+ ") is expected to contain even number of tokens ("
									+ roomSpecialismTokens.length
									+ ") after 4th pipeline (|) separated by a space ( ).");
				}
				List<RoomSpecialism> roomSpecialismListOfRoom = new ArrayList<RoomSpecialism>(
						roomSpecialismTokens.length / 2);
				for (int j = 0; j < roomSpecialismTokens.length; j += 2) {
					int priority = Integer.parseInt(roomSpecialismTokens[j]);
					long specialismId = Long
							.parseLong(roomSpecialismTokens[j + 1]);
					if (specialismId != 0) {
				...
				List<RoomEquipment> roomEquipmentListOfRoom = new ArrayList<RoomEquipment>(
						equipmentListSize);
		...
		private void readBedList() throws IOException {
			...
			Map<Room, Integer> roomToLastIndexInRoomMap = new HashMap<Room, Integer>(
					roomListSize);
			...
			Collections.sort(bedList, new Comparator<Bed>() {
				public int compare(Bed a, Bed b) {
					return new CompareToBuilder()
							.append(a.getRoom().getDepartment().getId(),
									b.getRoom().getDepartment().getId())
		...
		private void readPatientListAndAdmissionPartListAndRequiredPatientEquipmentListAndPreferredPatientEquipmentList()
			...
			List<AdmissionPart> admissionPartList = new ArrayList<AdmissionPart>(
					patientListSize);
			List<RequiredPatientEquipment> requiredPatientEquipmentList = new ArrayList<RequiredPatientEquipment>(
					patientListSize * equipmentListSize);
			List<PreferredPatientEquipment> preferredPatientEquipmentList = new ArrayList<PreferredPatientEquipment>(
					patientListSize * equipmentListSize);
			...
			for (int i = 0; i < patientListSize; i++) {
				...
				// A patient with no nights in the planning horizon or no nights
				// at all is ignored
				if (firstNightIndex >= nightListSize
						|| patientNightListSize == 0) {
					continue;
				}
				...
				int preferredMaximumRoomCapacity = Integer
						.parseInt(lineTokens[3]);
				patient.setPreferredMaximumRoomCapacity(preferredMaximumRoomCapacity == 0 ? null
						: preferredMaximumRoomCapacity);
				patientList.add(patient);

				String[] admissionPartTokens = splitBySpace(lineTokens[2]);
				int patientAdmissionPartListSize = Integer
						.parseInt(admissionPartTokens[0]);
				...
				for (int j = 1; j < admissionPartTokens.length; j += 2) {
					...
					int admissionPartNightListSize = Integer
							.parseInt(admissionPartTokens[j + 1]);
					if (nextFirstNightIndex >= nightListSize
							|| admissionPartNightListSize == 0) {
					...
					Specialism specialism = (specialismId == 0) ? null
					...
					Night admissionPartFirstNight = indexToNightMap
							.get(admissionPartFirstNightIndex);
					...
					int admissionPartLastNightIndex = nextFirstNightIndex
							+ admissionPartNightListSize - 1;
					// TODO Instead of ensureEnoughNights(lastNightIndex);
					// the official score function ignores any broken
					// constraints after the planning horizon
					if (admissionPartLastNightIndex >= nightListSize) {
						admissionPartLastNightIndex = nightListSize - 1;
					}
					Night admissionPartLastNight = indexToNightMap
							.get(admissionPartLastNightIndex);
				...
				int admissionPartNightListSizeSum = nextFirstNightIndex
						- firstNightIndex;
				...
				List<RequiredPatientEquipment> requiredPatientEquipmentOfPatientList = new ArrayList<RequiredPatientEquipment>(
						equipmentListSize);
				for (int j = 0; j < requiredPatientEquipmentTokens.length; j++) {
					int hasEquipment = Integer
							.parseInt(requiredPatientEquipmentTokens[j]);
					if (hasEquipment == 1) {
						RequiredPatientEquipment requiredPatientEquipment = new RequiredPatientEquipment();
						requiredPatientEquipment
								.setId(requiredPatientEquipmentId);
						requiredPatientEquipment.setPatient(patient);
						requiredPatientEquipment
								.setEquipment(indexToEquipmentMap.get(j));
						requiredPatientEquipmentOfPatientList
								.add(requiredPatientEquipment);
				...
				List<PreferredPatientEquipment> preferredPatientEquipmentOfPatientList = new ArrayList<PreferredPatientEquipment>(
						equipmentListSize);
				for (int j = 0; j < preferredPatientEquipmentTokens.length; j++) {
					int hasEquipment = Integer
							.parseInt(preferredPatientEquipmentTokens[j]);
					if (hasEquipment == 1) {
						boolean alreadyRequired = (Integer
								.parseInt(requiredPatientEquipmentTokens[j]) == 1);
						// Official spec: if equipment is required
						// then a duplicate preffered constraint should be
						// ignored
						if (!alreadyRequired) {
							PreferredPatientEquipment preferredPatientEquipment = new PreferredPatientEquipment();
							preferredPatientEquipment
									.setId(preferredPatientEquipmentId);
							preferredPatientEquipment.setPatient(patient);
							preferredPatientEquipment
									.setEquipment(indexToEquipmentMap.get(j));
							preferredPatientEquipmentOfPatientList
									.add(preferredPatientEquipment);
			...
			patientAdmissionSchedule
					.setRequiredPatientEquipmentList(requiredPatientEquipmentList);
			patientAdmissionSchedule
					.setPreferredPatientEquipmentList(preferredPatientEquipmentList);
		}
