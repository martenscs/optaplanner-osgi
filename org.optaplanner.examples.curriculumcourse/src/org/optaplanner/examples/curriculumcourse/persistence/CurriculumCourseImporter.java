...
public class CurriculumCourseImporter extends AbstractTxtSolutionImporter {
	...
	public static class CurriculumCourseInputBuilder extends TxtInputBuilder {
		...
		public Solution readSolution() throws IOException {
			...
			readUnavailablePeriodPenaltyList(schedule, courseMap, periodMap,
					unavailablePeriodPenaltyListSize);
			readEmptyLine();
			readConstantLine("END\\.");
			createLectureList(schedule);

			int possibleForOneLectureSize = schedule.getPeriodList().size()
					* schedule.getRoomList().size();
			BigInteger possibleSolutionSize = BigInteger.valueOf(
					possibleForOneLectureSize).pow(
					schedule.getLectureList().size());
			logger.info(
					"CourseSchedule {} has {} teachers, {} curricula, {} courses, {} lectures,"
							+ " {} periods, {} rooms and {} unavailable period constraints with a search space of {}.",
					getInputId(), schedule.getTeacherList().size(), schedule
							.getCurriculumList().size(), schedule
							.getCourseList().size(), schedule.getLectureList()
							.size(), schedule.getPeriodList().size(), schedule
							.getRoomList().size(), schedule
							.getUnavailablePeriodPenaltyList().size(),
		...
		private Map<String, Course> readCourseListAndTeacherList(
			...
			for (int i = 0; i < courseListSize; i++) {
				...
				course.setId((long) i);
				// Courses: <CourseID> <Teacher> <# Lectures> <MinWorkingDays>
				// <# Students>
		...
		private Teacher findOrCreateTeacher(Map<String, Teacher> teacherMap,
				String code) {
		...
		private Map<List<Integer>, Period> createPeriodListAndDayListAndTimeslotList(
				CourseSchedule schedule, int dayListSize, int timeslotListSize)
				throws IOException {
			int periodListSize = dayListSize * timeslotListSize;
			Map<List<Integer>, Period> periodMap = new HashMap<List<Integer>, Period>(
					periodListSize);
		...
		private void readCurriculumList(CourseSchedule schedule,
				Map<String, Course> courseMap, int curriculumListSize)
				throws IOException {
			readEmptyLine();
			...
			for (int i = 0; i < curriculumListSize; i++) {
				...
				// Curricula: <CurriculumID> <# Courses> <MemberID> ...
				// <MemberID>
				String line = bufferedReader.readLine();
		...
		private void readUnavailablePeriodPenaltyList(CourseSchedule schedule,
				Map<String, Course> courseMap,
				Map<List<Integer>, Period> periodMap,
				int unavailablePeriodPenaltyListSize) throws IOException {
			readEmptyLine();
			...
			for (int i = 0; i < unavailablePeriodPenaltyListSize; i++) {
				...
				Period period = periodMap.get(Arrays.asList(dayIndex,
						timeslotIndex));
				if (period == null) {
					throw new IllegalArgumentException("Read line (" + line
							+ ") uses an unexisting period(" + dayIndex + " "
							+ timeslotIndex + ").");
				}
