...
public class PredecessorsDoneDateUpdatingVariableListener implements
	...
	protected void updateAllocation(ScoreDirector scoreDirector,
		...
		uncheckedSuccessorQueue.addAll(originalAllocation
				.getSuccessorAllocationList());
		while (!uncheckedSuccessorQueue.isEmpty()) {
			Allocation allocation = uncheckedSuccessorQueue.remove();
			boolean updated = updatePredecessorsDoneDate(scoreDirector,
					allocation);
			if (updated) {
				uncheckedSuccessorQueue.addAll(allocation
						.getSuccessorAllocationList());
			}
		}
	}
	...
	protected boolean updatePredecessorsDoneDate(ScoreDirector scoreDirector,
		...
		Integer doneDate = 0;
		for (Allocation predecessorAllocation : allocation
