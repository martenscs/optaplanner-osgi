...
public class CheapTimeCostCalculator {
	...
	public static final BigDecimal MICROS_PER_ONE_AS_BIG_DECIMAL = new BigDecimal(
	...
	public static long parseMicroCost(String costString) {
		...
		if (costBigDecimal.scale() > 11) {
			throw new IllegalArgumentException("The costString (" + costString
					+ ") has a scale (" + costBigDecimal.scale()
					+ ") higher than 10.");
		}
		costBigDecimal = costBigDecimal.setScale(11);
		return costBigDecimal.multiply(MICROS_PER_ONE_AS_BIG_DECIMAL)
				.longValueExact();
	}

	public static long multiplyTwoMicros(long aMicros, long bMicros) {
		// Long arithmetic overflows because maxPowerConsumption
		// (675.4800000000) * maxPowerCost (0.0228608333)
