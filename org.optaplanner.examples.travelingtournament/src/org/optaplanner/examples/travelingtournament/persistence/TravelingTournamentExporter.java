...
public class TravelingTournamentExporter extends AbstractTxtSolutionExporter {
	...
	public static class TravelingTournamentOutputBuilder extends
			TxtOutputBuilder {
		...
		public void writeSolution() throws IOException {
			...
			for (Team team : travelingTournament.getTeamList()) {
				bufferedWriter.write(String.format("%-"
						+ (maximumTeamNameLength + 3) + "s", team.getName()));
			}
			...
			for (Day day : travelingTournament.getDayList()) {
				for (Team team : travelingTournament.getTeamList()) {
					...
					String opponentName = (opponentIsHome ? "@" : "")
							+ opponentTeam.getName();
					bufferedWriter.write(String.format("%-"
							+ (maximumTeamNameLength + 3) + "s", opponentName));
				}
				bufferedWriter.write("\n");
			}
		}
