...
public class TravelingTournamentImporter extends AbstractTxtSolutionImporter {
	...
	public static class TravelingTournamentInputBuilder extends TxtInputBuilder {
		...
		public Solution readSolution() throws IOException {
			...
			createMatchListAndSetDistancesInTeamList(travelingTournament,
					outerDistanceList);
			initializeMatchDays(travelingTournament);
			BigInteger possibleSolutionSize = factorial(2 * (n - 1)).pow(n / 2);
			logger.info(
					"TravelingTournament {} has {} days, {} teams and {} matches with a search space of {}.",
					getInputId(), travelingTournament.getDayList().size(),
		...
		private void readTeamList(TravelingTournament travelingTournament, int n)
				throws IOException {
		...
		private List<List<Integer>> readOuterDistanceList(
				TravelingTournament travelingTournament) throws IOException {
		...
		private void createDayList(TravelingTournament travelingTournament,
				int n) {
			List<Day> dayList = new ArrayList<Day>();
			int daySize = (n - 1) * 2; // Play vs each team (except itself)
										// twice (home and away)
		...
		private void initializeMatchPairs(
				TravelingTournament travelingTournament, int a, int b, int i) {
			...
			Match m1 = findMatch(travelingTournament.getMatchList(), aTeam,
					bTeam);
			m1.setDay(travelingTournament.getDayList().get(i));
			Match m2 = findMatch(travelingTournament.getMatchList(), bTeam,
					aTeam);
			m2.setDay(travelingTournament.getDayList().get(
					i + travelingTournament.getN() - 1));
		}

		private Match findMatch(List<Match> matchList, Team homeTeam,
				Team awayTeam) {
			...
			throw new IllegalStateException("Nothing found for: " + homeTeam
					+ " and " + awayTeam);
		}
