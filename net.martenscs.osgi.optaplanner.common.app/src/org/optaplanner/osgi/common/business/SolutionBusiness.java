...
package org.optaplanner.osgi.common.business;
...
public class SolutionBusiness {
	...
	public void updateDataDirs() {
		...
		if (hasImporter()) {
			...
			if (!importDataDir.exists()) {
				throw new IllegalStateException("The directory importDataDir ("
						+ importDataDir.getAbsolutePath() + ") does not exist.");
			}
		}
		...
		if (!solvedDataDir.exists() && !solvedDataDir.mkdir()) {
			throw new IllegalStateException("The directory solvedDataDir ("
		...
		if (hasExporter()) {
			...
			if (!exportDataDir.exists() && !exportDataDir.mkdir()) {
				throw new IllegalStateException("The directory exportDataDir ("
	...
	public List<File> getSolvedFileList() {
		List<File> fileList = new ArrayList<File>(FileUtils.listFiles(
				solvedDataDir, new String[] { solutionDao.getFileExtension() },
				true));
		Collections.sort(fileList, FILE_COMPARATOR);
		return fileList;
	}
	...
	// public void registerForBestSolutionChanges(final
	...
	// {
	// // Avoid ConcurrentModificationException when there is an unprocessed
	// ProblemFactChange
	// // because the paint method uses the same problem facts instances as the
	// Solver's workingSolution
	// // unlike the planning entities of the bestSolution which are cloned from
	// the Solver's workingSolution
	...
	// // public void run() {
	// // // TODO by the time we process this event, a newer
	// // bestSolution might already be queued
	...
	@SuppressWarnings("rawtypes")
	public void openSolution(URL file) {
		Solution solution = solutionDao.readSolution(file);
		solutionFileName = file.getFile();
		guiScoreDirector.setWorkingSolution(solution);
	}
	...
	public void doMove(Move move) {
		...
		if (!move.isMoveDoable(guiScoreDirector)) {
			logger.warn("Not doing user move ({}) because it is not doable.",
					move);
			return;
		}
	...
	public ChangeMove createChangeMove(Object entity, String variableName,
			Object toPlanningValue) {
		SolutionDescriptor solutionDescriptor = ((InnerScoreDirector) guiScoreDirector)
				.getSolutionDescriptor();
		GenuineVariableDescriptor variableDescriptor = solutionDescriptor
				.findGenuineVariableDescriptorOrFail(entity, variableName);
		if (variableDescriptor.isChained()) {
			return new ChainedChangeMove(entity, variableDescriptor,
					toPlanningValue);
		} else {
	...
	public void doChangeMove(Object entity, String variableName,
			Object toPlanningValue) {
		ChangeMove move = createChangeMove(entity, variableName,
				toPlanningValue);
		doMove(move);
	}
	...
	public SwapMove createSwapMove(Object leftEntity, Object rightEntity) {
		...
		EntityDescriptor entityDescriptor = solutionDescriptor
				.findEntityDescriptor(leftEntity.getClass());
		...
		if (entityDescriptor.hasAnyChainedGenuineVariables()) {
			return new ChainedSwapMove(variableDescriptors, leftEntity,
					rightEntity);
		} else {
