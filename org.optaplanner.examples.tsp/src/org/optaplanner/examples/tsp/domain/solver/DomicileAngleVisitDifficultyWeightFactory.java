 ...
 */
public class DomicileAngleVisitDifficultyWeightFactory implements
		SelectionSorterWeightFactory<TravelingSalesmanTour, Visit> {

	@SuppressWarnings("rawtypes")
	public Comparable createSorterWeight(
			TravelingSalesmanTour vehicleRoutingSolution, Visit visit) {
		Domicile domicile = vehicleRoutingSolution.getDomicile();
		return new DomicileAngleVisitDifficultyWeight(visit, visit
				.getLocation().getAngle(domicile.getLocation()), visit
				.getLocation().getDistance(domicile.getLocation())
				+ domicile.getLocation().getDistance(visit.getLocation()));
	}

	public static class DomicileAngleVisitDifficultyWeight implements
			Comparable<DomicileAngleVisitDifficultyWeight> {
		...
		public int compareTo(DomicileAngleVisitDifficultyWeight other) {
			return new CompareToBuilder()
					...
					.append(domicileRoundTripDistance,
							other.domicileRoundTripDistance) // Ascending
																// (further from
																// the depot are
																// more
																// difficult)
					.append(visit.getId(), other.visit.getId()).toComparison();
		}
