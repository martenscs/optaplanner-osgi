	...
	public static class TspInputBuilder extends TxtInputBuilder {
		...
		public Solution readSolution() throws IOException {
			...
			if (firstLine.matches("\\s*NAME\\s*:.*")) {
				travelingSalesmanTour.setName(removePrefixSuffixFromLine(
						firstLine, "\\s*NAME\\s*:", ""));
				readTspLibFormat();
			} else {
				travelingSalesmanTour.setName(FilenameUtils
						.getBaseName(inputFile.getName()));
				locationListSize = Integer.parseInt(firstLine.trim());
				readCourseraFormat();
			}
			BigInteger possibleSolutionSize = factorial(travelingSalesmanTour
					.getLocationList().size() - 1);
			logger.info(
					"TravelingSalesmanTour {} has {} locations with a search space of {}.",
					getInputId(), travelingSalesmanTour.getLocationList()
							.size(),
		...
		private void readTspLibHeaders() throws IOException {
			...
			if (edgeWeightType.equalsIgnoreCase("EUC_2D")) {
				travelingSalesmanTour
						.setDistanceType(DistanceType.AIR_DISTANCE);
			} else if (edgeWeightType.equalsIgnoreCase("EXPLICIT")) {
				travelingSalesmanTour
						.setDistanceType(DistanceType.ROAD_DISTANCE);
			...
			travelingSalesmanTour
					.setDistanceUnitOfMeasurement(readOptionalStringValue(
				...
				for (int i = 0; i < locationListSize; i++) {
					...
					for (int j = 0; j < locationListSize; j++) {
						...
						if (i == j) {
							...
							RoadLocation otherLocation = (RoadLocation) locationList
									.get(j);
							travelDistanceMap
		...
		private void createVisitList() {
			List<Location> locationList = travelingSalesmanTour
					.getLocationList();
