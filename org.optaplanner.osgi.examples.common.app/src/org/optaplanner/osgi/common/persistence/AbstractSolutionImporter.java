...
public abstract class AbstractSolutionImporter extends LoggingMain {
	...
	public void convertAll() {
		...
		for (File inputFile : inputFiles) {
			if (acceptInputFile(inputFile)
					&& acceptInputFileDuringBulkConvert(inputFile)) {
	...
	public void convert(String inputFileName, String outputFileName) {
		...
		if (!inputFile.exists()) {
			throw new IllegalStateException("The file inputFile ("
	...
	public static String getFlooredPossibleSolutionSize(
		...
		return "10^"
				+ (BigIntegerMath.log10(possibleSolutionSize,
						RoundingMode.FLOOR));
	}
