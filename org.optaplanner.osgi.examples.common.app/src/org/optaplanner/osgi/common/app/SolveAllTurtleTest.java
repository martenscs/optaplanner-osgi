...
/**
 * Turtle tests are not run by default. They are only run if
 * <code>-DrunTurtleTests=true</code> because it takes days.
 */
@SuppressWarnings("rawtypes")
public abstract class SolveAllTurtleTest extends LoggingMain {
	...
	public void runFastAndFullAssert() {
		...
		// Specifically use NON_INTRUSIVE_FULL_ASSERT instead of FULL_ASSERT to
		// flush out bugs hidden by intrusiveness
		// 1) NON_INTRUSIVE_FULL_ASSERT ASSERT to find CH bugs (but covers
		// little ground)
		planningProblem = buildAndSolve(solverFactory,
				EnvironmentMode.NON_INTRUSIVE_FULL_ASSERT, planningProblem, 2L);
		// 2) FAST_ASSERT to run past CH into LS to find easy bugs (but covers
		// much ground)
		planningProblem = buildAndSolve(solverFactory,
				EnvironmentMode.FAST_ASSERT, planningProblem, 5L);
		// 3) NON_INTRUSIVE_FULL_ASSERT ASSERT to find LS bugs (but covers
		// little ground)
		planningProblem = buildAndSolve(solverFactory,
				EnvironmentMode.NON_INTRUSIVE_FULL_ASSERT, planningProblem, 3L);
	}

	protected Solution buildAndSolve(SolverFactory solverFactory,
			EnvironmentMode environmentMode, Solution planningProblem,
			long maximumMinutesSpent) {
		SolverConfig solverConfig = solverFactory.getSolverConfig();
		solverConfig.getTerminationConfig().setMinutesSpentLimit(
				maximumMinutesSpent);
		...
		if (easyScoreCalculatorClass != null && environmentMode.isAsserted()) {
			...
			assertionScoreDirectorFactoryConfig
					.setEasyScoreCalculatorClass(easyScoreCalculatorClass);
		...
		if (bestSolution == null) {
			// Solver didn't make it past initialization // TODO remove me once
			// getBestSolution() never returns null
