	...
	public static class DinnerPartyInputBuilder extends TxtInputBuilder {
		...
		public Solution readSolution() throws IOException {
			...
			BigInteger possibleSolutionSize = BigInteger.valueOf(
					dinnerParty.getGuestList().size()).pow(
					dinnerParty.getSeatDesignationList().size());
			logger.info(
					"DinnerParty {} has {} jobs, {} guests, {} hobby practicians, {} tables and {} seats"
							+ " with a search space of {}.", getInputId(),
					dinnerParty.getJobList().size(), dinnerParty.getGuestList()
							.size(), dinnerParty.getHobbyPracticianList()
							.size(), dinnerParty.getTableList().size(),
		...
		private void readTableListAndSeatList(DinnerParty dinnerParty)
			...
			List<Seat> seatList = new ArrayList<Seat>(tableListSize
					* seatsPerTable);
		...
		private void readJobListGuestListAndHobbyPracticianList(
				DinnerParty dinnerParty) throws IOException {
			...
			List<HobbyPractician> hobbyPracticianList = new ArrayList<HobbyPractician>(
					guestSize * 3);
			Map<String, Job> jobMap = new HashMap<String, Job>(
					JobType.values().length * 5);
			int jobNextId = 0;
			int hobbyPracticianJobId = 0;
			for (int i = 0; i < guestSize; i++) {
				Guest guest = new Guest();
				guest.setId((long) i);
				String[] lineTokens = splitByCommaAndTrim(
						bufferedReader.readLine(), 6, null);
				...
				List<HobbyPractician> hobbyPracticianOfGuestList = new ArrayList<HobbyPractician>(
						lineTokens.length - 5);
