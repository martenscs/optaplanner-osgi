...
public class DinnerPartySolutionInitializer implements CustomPhaseCommand {
	...
	public void changeWorkingSolution(ScoreDirector scoreDirector) {
		DinnerParty dinnerParty = (DinnerParty) scoreDirector
				.getWorkingSolution();
		initializeSeatDesignationList(scoreDirector, dinnerParty);
	}

	@SuppressWarnings({ "rawtypes", "unchecked" })
	private void initializeSeatDesignationList(ScoreDirector scoreDirector,
			DinnerParty dinnerParty) {
		// TODO the planning entity list from the solution should be used and
		// might already contain initialized entities
		List<SeatDesignation> seatDesignationList = createSeatDesignationList(dinnerParty);
		// Assign one guest at a time
		List<Seat> undesignatedSeatList = new ArrayList<Seat>(
				dinnerParty.getSeatList());
		for (SeatDesignation seatDesignation : seatDesignationList) {
			...
			// TODO by reordening the seats so index 0 has a different table
			// then index 1 and so on,
			// this will probably be faster because perfectMatch will be true
			// sooner
			for (Seat seat : undesignatedSeatList) {
				if (seatDesignation.getGuest().getGender() == seat
						.getRequiredGender()) {
					if (!added) {
						...
						scoreDirector.beforeVariableChanged(seatDesignation,
								"seat");
						seatDesignation.setSeat(seat);
						scoreDirector.afterVariableChanged(seatDesignation,
								"seat");
					}
			...
			if (bestSeat == null) {
				throw new IllegalStateException("The bestSeat (" + bestSeat
						+ ") cannot be null.");
			}
			...
			// There will always be enough allowed seats: ok to do this for this
			// problem, but not ok for most problems
	...
	private List<SeatDesignation> createSeatDesignationList(
			DinnerParty dinnerParty) {
		List<SeatDesignation> seatDesignationList = new ArrayList<SeatDesignation>(
