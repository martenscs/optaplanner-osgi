...
public class MachineReassignmentImporter extends AbstractTxtSolutionImporter {
	...
	public boolean acceptInputFileDuringBulkConvert(File inputFile) {
		...
		return !Arrays.asList("model_b_1.txt", "model_b_2.txt",
				"model_b_3.txt", "model_b_4.txt", "model_b_5.txt",
				"model_b_6.txt", "model_b_7.txt", "model_b_8.txt",
				"model_b_9.txt", "model_b_10.txt")
				.contains(inputFile.getName());
	}
	...
	public static class MachineReassignmentInputBuilder extends TxtInputBuilder {
		...
		public Solution readSolution() throws IOException {
			...
			BigInteger possibleSolutionSize = BigInteger.valueOf(
					machineReassignment.getMachineList().size()).pow(
					machineReassignment.getProcessList().size());
			logger.info(
					"MachineReassignment {} has {} resources, {} neighborhoods, {} locations, {} machines,"
							+ " {} services, {} processes and {} balancePenalties with a search space of {}.",
					getInputId(), machineReassignment.getResourceList().size(),
		...
		private void readMachineList() throws IOException {
			...
			List<MrNeighborhood> neighborhoodList = new ArrayList<MrNeighborhood>(
					machineListSize);
			Map<Long, MrNeighborhood> idToNeighborhoodMap = new HashMap<Long, MrNeighborhood>(
					machineListSize);
			List<MrLocation> locationList = new ArrayList<MrLocation>(
					machineListSize);
			Map<Long, MrLocation> idToLocationMap = new HashMap<Long, MrLocation>(
					machineListSize);
			machineList = new ArrayList<MrMachine>(machineListSize);
			long machineId = 0L;
			List<MrMachineCapacity> machineCapacityList = new ArrayList<MrMachineCapacity>(
					machineListSize * resourceListSize);
			long machineCapacityId = 0L;
			// 2 phases because service dependencies are not in low to high
			// order
			...
			for (int i = 0; i < machineListSize; i++) {
				...
				String[] lineTokens = splitBySpace(line, moveCostOffset
						+ machineListSize);
				long neighborhoodId = Long.parseLong(lineTokens[0]);
				MrNeighborhood neighborhood = idToNeighborhoodMap
						.get(neighborhoodId);
				...
				List<MrMachineCapacity> machineCapacityListOfMachine = new ArrayList<MrMachineCapacity>(
						resourceListSize);
				for (int j = 0; j < resourceListSize; j++) {
					...
					machineCapacity.setMaximumCapacity(Long
							.parseLong(lineTokens[2 + j]));
					machineCapacity.setSafetyCapacity(Long
							.parseLong(lineTokens[2 + resourceListSize + j]));
				...
				Map<MrMachine, Integer> machineMoveCostMap = new HashMap<MrMachine, Integer>(
						machineListSize);
				for (int j = 0; j < machineListSize; j++) {
					MrMachine toMachine = machineList.get(j);
					int moveCost = Integer.parseInt(lineTokens[moveCostOffset
							+ j]);
		...
		private void readServiceList() throws IOException {
			...
			for (int i = 0; i < serviceListSize; i++) {
				...
				List<MrService> toDependencyServiceList = new ArrayList<MrService>(
						serviceDependencyListSize);
				for (int j = 0; j < serviceDependencyListSize; j++) {
					...
					if (toServiceIndex >= serviceList.size()) {
						throw new IllegalArgumentException("Service with id ("
								+ serviceId
								+ ") has a non existing toServiceIndex ("
								+ toServiceIndex + ").");
					}
					MrService toService = serviceList.get(toServiceIndex);
					if (toService.equals(service)) {
						throw new IllegalStateException("The toService ("
								+ toService
								+ ") cannot be equal to the service ("
								+ service + ").");
					}
				...
				if (lineTokens.length != numberOfTokens) {
					throw new IllegalArgumentException("Read line (" + line
							+ ") has " + lineTokens.length
							+ " tokens but is expected to contain "
							+ numberOfTokens + " tokens separated by space.");
				}
			}
		...
		private void readProcessList() throws IOException {
			...
			for (int i = 0; i < processListSize; i++) {
				...
				if (serviceIndex >= serviceList.size()) {
					throw new IllegalArgumentException("Process with id ("
							+ processId + ") has a non existing serviceIndex ("
							+ serviceIndex + ").");
				}
				MrService service = serviceList.get(serviceIndex);
				process.setService(service);
				List<MrProcessRequirement> processRequirementList = new ArrayList<MrProcessRequirement>(
						resourceListSize);
				for (int j = 0; j < resourceListSize; j++) {
					...
					processRequirement.setUsage(Integer
							.parseInt(lineTokens[1 + j]));
				...
				process.setMoveCost(Integer
						.parseInt(lineTokens[1 + resourceListSize]));
		...
		private void readBalancePenaltyList() throws IOException {
			...
			List<MrBalancePenalty> balancePenaltyList = new ArrayList<MrBalancePenalty>(
					balancePenaltyListSize);
			...
			for (int i = 0; i < balancePenaltyListSize; i++) {
				...
				if (originResourceIndex >= resourceListSize) {
					throw new IllegalArgumentException(
							"BalancePenalty with id ("
									+ balancePenaltyId
									+ ") has a non existing originResourceIndex ("
									+ originResourceIndex + ").");
				}
				balancePenalty.setOriginResource(resourceList
						.get(originResourceIndex));
				int targetResourceIndex = Integer.parseInt(lineTokens[1]);
				if (targetResourceIndex >= resourceListSize) {
					throw new IllegalArgumentException(
							"BalancePenalty with id ("
									+ balancePenaltyId
									+ ") has a non existing targetResourceIndex ("
									+ targetResourceIndex + ").");
				}
		...
		private void readGlobalPenaltyInfo() throws IOException {
			...
			String line = readStringValue();
			String[] lineTokens = splitBySpace(line, 3);
			globalPenaltyInfo.setProcessMoveCostWeight(Integer
					.parseInt(lineTokens[0]));
			globalPenaltyInfo.setServiceMoveCostWeight(Integer
					.parseInt(lineTokens[1]));
			globalPenaltyInfo.setMachineMoveCostWeight(Integer
		...
		private void readProcessAssignmentList() {
			...
			List<MrProcessAssignment> processAssignmentList = new ArrayList<MrProcessAssignment>(
					processListSize);
			...
			for (int i = 0; i < processListSize; i++) {
				...
				if (machineIndex >= machineList.size()) {
					throw new IllegalArgumentException(
							"ProcessAssignment with id (" + processAssignmentId
									+ ") has a non existing machineIndex ("
									+ machineIndex + ").");
				}
				processAssignment.setOriginalMachine(machineList
						.get(machineIndex));
		...
		private String readOriginalProcessAssignmentLine() {
			...
			if (!inputFileName.startsWith(inputFilePrefix)) {
				throw new IllegalArgumentException("The inputFile ("
						+ inputFile + ") is expected to start with \""
						+ inputFilePrefix + "\".");
			}
			...
			try {
				assignmentBufferedReader = new BufferedReader(
						new InputStreamReader(new FileInputStream(
								assignmentInputFile), "UTF-8"));
				try {
					return assignmentBufferedReader.readLine();
				} catch (IllegalArgumentException e) {
					throw new IllegalArgumentException(
							"Exception in assignmentInputFile ("
